...
const XANO_API_BASE =
  process.env.NEXT_PUBLIC_XANO_API_URL || 'https://your-instance.xano.io/api:your-api';
...
async function xanoFetch(endpoint: string, options: RequestInit = {}) {
  ...
  // Only attach Authorization if key exists (prevents "Bearer undefined")
  ...
  if (!response.ok) {
    ...
    throw new Error(`Xano API error: ${response.status}${text ? ` - ${text}` : ''}`);
  }
