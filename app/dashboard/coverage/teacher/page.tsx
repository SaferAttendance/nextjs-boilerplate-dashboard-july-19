...
export default async function TeacherCoveragePage() {
  ...
  if (normalizedRole !== 'teacher') {
    // Non-teachers should go to their appropriate view
  ...
  const teacherData = {
    ...
    department: 'Mathematics', // Would come from DB
    employeeId: 'T-' + Math.random().toString(36).substr(2, 9).toUpperCase(),
    monthlyEarnings: 127.50,
    yearToDateEarnings: 1847.50,
    hoursThisMonth: 8.5,
    pendingApproval: 67.50,
    approvedAmount: 60.00,
    todaysCoverage: {
      substitute: 'Mike Thompson',
      classes: [
        { period: 2, name: 'Math 101', room: '204' },
        { period: 4, name: 'Algebra II', room: '204' },
        { period: 6, name: 'Geometry', room: '204' },
      ],
    },
  };
        ...
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">Teacher View</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="hidden sm:flex items-center space-x-2">
              ...
              <span className="text-sm text-gray-600">
                {profileFullName} | {teacherData.department}
              </span>
            </div>
          </div>
        </div>
